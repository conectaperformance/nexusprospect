...
serve(async (req) => {
    ...
    try {
        ...
        // 6. Mapear os dados recebidos da Extensão do Google Maps para o formato do banco de dados
        const mappedLeads = incomingLeads.map((item) => ({
            user_id: userId,
            name: item.nome_empresa || 'Sem Nome',
            company: item.nome_empresa || null,
            ...
            source: 'google_maps',
            search_term: item.search_term || null, // A extensão atual ainda não manda o termo de busca, mas previnemmos
        }))
